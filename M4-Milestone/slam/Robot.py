...
class Robot:
    ...
    def drive(self, drive_meas):
        ...
        if (drive_meas.left_speed == drive_meas.right_speed):
            ...
            theta_next = self.state[2] + angular_velocity*dt
            self.state[0] = self.state[0] + R*(-np.sin(self.state[2]) + np.sin(theta_next))
            self.state[1] = self.state[1] + R*(np.cos(self.state[2]) - np.cos(theta_next))
            self.state[2] = theta_next
