...
class Slam:
    ...
    def save_map(self, fname="slam_map.json"):
    ...
    def predict(self, raw_drive_meas):
        self.robot.drive(raw_drive_meas) ##Predicted State:
        A = self.state_transition(raw_drive_meas)
        self.P = A@self.P@A.T +self.predict_covariance(raw_drive_meas)
    ...
    def update(self, measurements):
        ...
        #Estimated State via z_hat
        x += kalman_gain@(z - z_hat)
        self.set_state_vector(x) #Setting the newly updated
        #Estiamted Covariance
        temp = kalman_gain@H
        size = temp.shape[0]
        self.P = (np.identity(size) - temp)@self.P
